...
import com.alonso.rockpaperscissors.views.Scores
...
class MainActivity : ComponentActivity() {
    companion object {
        lateinit var database: VictoriaDB
        lateinit var scores : List<VictoriaEntity>
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        database = Room.databaseBuilder(
            this, VictoriaDB::class.java, "victoria-db"
        ).build()
        enableEdgeToEdge()
        setContent {
            coroutine = rememberCoroutineScope()
            val navController = rememberNavController()
            NavHost(navController = navController, startDestination =  "RPS") {
                composable("RPS") {
                    RPS(Modifier.fillMaxSize(), navController)
                }
                composable("scores") {
                    Scores(Modifier.fillMaxSize())
                }
            }
